...
class CollectionViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_context(self):
        ...
        if self.action not in ["list", "all", "archived", "shared"]:
            is_nested = (
                self.request.query_params.get("nested", "false").lower() == "true"
            )
            if is_nested:
                context["nested"] = True
    ...
    def get_optimized_queryset_for_listing(self, queryset):
        """Apply shared list-action optimizations on top of a filtered queryset."""
        return queryset.select_related("user", "primary_image").prefetch_related(
            Prefetch(
                "locations__images",
                queryset=ContentImage.objects.filter(is_primary=True).select_related(
                    "user"
                ),
    ...
    def get_queryset(self):
        """Get base queryset for non-custom actions handled by ModelViewSet."""
    ...
    def list(self, request):
        ...
        queryset = self.get_optimized_queryset_for_listing(
    ...
    def all(self, request):
        ...
        queryset = self.get_optimized_queryset_for_listing(
            Collection.objects.filter(Q(user=request.user))
        )
    ...
    def archived(self, request):
        ...
        queryset = self.get_optimized_queryset_for_listing(
            Collection.objects.filter(Q(user=request.user.id) & Q(is_archived=True))
        )
    ...
    def retrieve(self, request, pk=None):
        ...
        itinerary_serializer = CollectionItineraryItemSerializer(
    ...
    def shared(self, request):
        ...
        queryset = self.get_optimized_queryset_for_listing(
            Collection.objects.filter(shared_with=request.user)
        )
    ...
    def share(self, request, pk=None, uuid=None):
        ...
        if collection.shared_with.filter(id=user.id).exists():
            return Response(
                {"error": "Collection is already shared with this user"}, status=400
            )

        # Check if there's already a pending invite for this user
        if CollectionInvite.objects.filter(
            collection=collection, invited_user=user
        ).exists():
    ...
    def revoke_invite(self, request, pk=None, uuid=None):
        ...
        if collection.user != request.user:
            return Response(
                {"error": "Only collection owner can revoke invites"}, status=403
            )
        ...
        try:
            ...
            return Response(
                {"error": "No pending invite found for this user"}, status=404
            )
    ...
    def accept_invite(self, request, pk=None):
        ...
        try:
            invite = CollectionInvite.objects.get(
                collection=collection, invited_user=request.user
            )
        except CollectionInvite.DoesNotExist:
            return Response(
                {"error": "No pending invite found for this collection"}, status=404
            )
        ...
        return Response(
            {"success": f"Successfully joined collection: {collection.name}"}
        )
    ...
    def decline_invite(self, request, pk=None):
        ...
        try:
            ...
            invite.delete()
            return Response(
                {"success": f"Declined invite for collection: {collection.name}"}
            )
        except CollectionInvite.DoesNotExist:
            return Response(
                {"error": "No pending invite found for this collection"}, status=404
    ...
    def can_share(self, request, pk=None):
        ...
        users_with_pending_invites = set(
            str(uuid)
            for uuid in CollectionInvite.objects.filter(
                collection=collection
            ).values_list("invited_user__uuid", flat=True)
        )
        users_already_shared = set(
            str(uuid) for uuid in collection.shared_with.values_list("uuid", flat=True)
        )
    ...
    def unshare(self, request, pk=None, uuid=None):
        ...
        if not collection.shared_with.filter(id=user.id).exists():
            return Response(
                {"error": "Collection is not shared with this user"}, status=400
            )
    ...
    def leave(self, request, pk=None):
        ...
        if request.user == collection.user:
            return Response(
                {"error": "Owner cannot leave their own collection"}, status=400
            )

        if not collection.shared_with.filter(id=request.user.id).exists():
            return Response(
                {"error": "You are not a member of this collection"}, status=400
            )
    ...
    def export_collection(self, request, pk=None):
        ...
        for loc in collection.locations.all().select_related(
            "city", "region", "country"
        ):
            loc_entry = {
                "id": str(loc.id),
                ...
                export_data["attachments"].append(
                    {
                        ...
                        "name": os.path.basename(
                            getattr(att.file, "name", "attachment")
                        ),
                    }
                )
        ...
        if collection.primary_image:
            export_data["primary_image_ref"] = image_export_map.get(
                str(collection.primary_image.id)
            )
    ...
    def import_collection(self, request):
        ...
        with zipfile.ZipFile(io.BytesIO(file_bytes), "r") as zipf:
            try:
                ...
                return Response(
                    {"detail": "metadata.json missing"},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            base_name = (metadata.get("collection") or {}).get(
                "name"
            ) or "Imported Collection"

            # Ensure unique name per user
            existing_names = set(
                request.user.collection_set.values_list("name", flat=True)
            )
            ...
            new_collection = Collection.objects.create(
                ...
                start_date=__import__("datetime").date.fromisoformat(
                    (metadata.get("collection") or {}).get("start_date")
                )
                if (metadata.get("collection") or {}).get("start_date")
                else None,
                end_date=__import__("datetime").date.fromisoformat(
                    (metadata.get("collection") or {}).get("end_date")
                )
                if (metadata.get("collection") or {}).get("end_date")
                else None,
                link=(metadata.get("collection") or {}).get("link"),
            )

            image_export_map = {
                img["export_id"]: img for img in metadata.get("images", [])
            }
            attachment_export_map = {
                att["export_id"]: att for att in metadata.get("attachments", [])
            }
            ...
            for loc_data in metadata.get("locations", []):
                ...
                if loc_data.get("category"):
                    cat_obj, _ = Category.objects.get_or_create(
                        user=request.user, name=loc_data["category"]
                    )
                ...
                def _coords_close(lat1, lon1, lat2, lon2, threshold=0.02):
                    try:
                        if lat1 is None or lon1 is None or lat2 is None or lon2 is None:
                            return False
                        return (
                            abs(float(lat1) - float(lat2)) <= threshold
                            and abs(float(lon1) - float(lon2)) <= threshold
                        )
                    except Exception:
                        return False
                ...
                for cand in Location.objects.filter(user=request.user):
                    ...
                    loc_text_score = _ratio(
                        incoming_location_text, getattr(cand, "location", None)
                    )
                    close_coords = _coords_close(
                        incoming_lat, incoming_lon, cand.latitude, cand.longitude
                    )
                    # Define "very similar": strong name match OR decent name with location/coords match
                    combined_score = max(
                        name_score, (name_score + loc_text_score) / 2.0
                    )
                    if close_coords:
                        combined_score = max(
                            combined_score, name_score + 0.1
                        )  # small boost for coord proximity
                    if combined_score > best_score and (
                        name_score >= 0.92
                        or (
                            name_score >= 0.85
                            and (loc_text_score >= 0.85 or close_coords)
                        )
                    ):
                ...
                if created_new_loc:
                    for export_id in loc_data.get("images", []):
                        ...
                        member = next(
                            (m for m in zipf.namelist() if m.startswith(prefix)), None
                        )
                        if not member:
                            continue
                    ...
                    for export_id in loc_data.get("attachments", []):
                        ...
                        member = next(
                            (
                                ...
                                if m == f"attachments/{file_name_att}"
                            ),
                            None,
                        )
                        if not member:
    ...
    def duplicate(self, request, pk=None):
        ...
        try:
            with transaction.atomic():
                ...
                if original.primary_image:
                    ...
                    if original_primary.image:
                        ...
                        file_name = (original_primary.image.name or "").split("/")[
                            -1
                        ] or "image.webp"
                ...
                def _copy_generic_media(source_obj, target_obj):
                    ...
                    for img in source_obj.images.all():
                        if img.image:
                            ...
                            file_name = (img.image.name or "").split("/")[
                                -1
                            ] or "image.webp"
                            media = ContentImage(
                                user=request.user,
                    ...
                    for attachment in source_obj.attachments.all():
                        ...
                        file_name = (attachment.file.name or "").split("/")[
                            -1
                        ] or "attachment"
            ...
            logging.getLogger(__name__).exception(
                "Failed to duplicate collection %s", pk
            )
            return Response(
    ...
    def _cleanup_out_of_range_itinerary_items(self, collection):
        ...
        deleted_items = (
            CollectionItineraryItem.objects.filter(collection=collection)
            .exclude(date__range=[collection.start_date, collection.end_date])
            .delete()
        )

        # Delete day metadata outside the date range
        deleted_days = (
            CollectionItineraryDay.objects.filter(collection=collection)
            .exclude(date__range=[collection.start_date, collection.end_date])
            .delete()
        )
    ...
    def update(self, request, *args, **kwargs):
        ...
        if is_public_changed:
            if new_is_public:
                ...
                serializer.instance.locations.filter(is_public=False).update(
                    is_public=True
                )
                serializer.instance.transportation_set.filter(is_public=False).update(
                    is_public=True
                )
                serializer.instance.note_set.filter(is_public=False).update(
                    is_public=True
                )
                serializer.instance.checklist_set.filter(is_public=False).update(
                    is_public=True
                )
                serializer.instance.lodging_set.filter(is_public=False).update(
                    is_public=True
                )
            else:
                ...
                locations_in_collection = serializer.instance.locations.filter(
                    is_public=True
                )
                for location in locations_in_collection:
                    # Check if this location belongs to any other public collection
                    has_other_public_collection = (
                        location.collections.filter(is_public=True)
                ...
                # Transportation
                transportations_to_check = (
                    serializer.instance.transportation_set.filter(is_public=True)
                )
                ...
                # Checklists
                checklists_to_check = serializer.instance.checklist_set.filter(
                    is_public=True
                )
                ...
                # Lodging
                lodging_to_check = serializer.instance.lodging_set.filter(
                    is_public=True
                )
